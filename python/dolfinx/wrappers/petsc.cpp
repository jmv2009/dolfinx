...
void petsc_module_fem(py::module& m)
{
...
void petsc_module_la(py::module& m)
{
  m.def("create_vector",
...
void petsc_module_nls(py::module& m)
{
  py::module m_petsc
...
{
  py::module petsc_mod_fem
      = m_fem.def_submodule("petsc", "PETSc-specific finite element module");
  petsc_module_fem(petsc_mod_fem);

  py::module petsc_mod_la
      = m_la.def_submodule("petsc", "PETSc-specific linear algebra");
  petsc_module_la(petsc_mod_la);

  // Main module, not a submodule, since PETSc is only NLS implementation
  petsc_module_nls(m_nls);
}
