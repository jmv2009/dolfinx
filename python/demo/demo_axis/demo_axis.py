...
from dolfinx import default_scalar_type, has_petsc
...
for m in m_list:
    ...
    if has_petsc:
        problem = LinearProblem(
            a, L, bcs=[], u=Esh_m, petsc_options={"ksp_type": "preonly", "pc_type": "lu"}
        )
    else:
    ...
    if m == 0:  # initialize and do not add 2 factor
        ...
        q_abs_fenics_proc = (
            fem.assemble_scalar(fem.form(Q * rho * dAu, dtype=complex_dtype)) / gcs / I0
        ).real
        q_sca_fenics_proc = (
            fem.assemble_scalar(
                fem.form((P("+") + P("-")) * rho * dS(scatt_tag), dtype=complex_dtype)
            )
            / gcs
            / I0
        ).real
...
if has_vtx:
    ...
    print(Esh, type(Esh), W.element.interpolation_points().dtype, complex_dtype)
    pts = W.element.interpolation_points().astype(default_real_type)
    Es_expr = fem.Expression(Esh, pts, dtype=complex_dtype)
